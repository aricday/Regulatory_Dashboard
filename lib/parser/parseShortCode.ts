import type { ShortCodeRow } from '@/types/spreadsheet';
import { normalizeCountry } from './normalizeCountry';
import type { WorkSheet } from 'xlsx';
import { utils } from 'xlsx';

/**
 * Short Code PN Types tab columns (verified from Weather Report 2026-04-10):
 *   0: Country
 *   1: Type ("Short Code")
 *   2: Capability ("SMS")
 *   3: Current Status
 ...
 */
export function parseShortCode(sheet: WorkSheet): { rows: ShortCodeRow[]; warnings: string[] } {
  const raw = utils.sheet_to_json<unknown[]>(sheet, { header: 1 });
  const rows: ShortCodeRow[] = [];
  ...
  for (let i = 1; i < raw.length; i++) {
    ...
    if (!isoCode) {
      warnings.push(`Short Code row ${i + 1}: Could not resolve country "${countryName}"`);
      continue;
    }
